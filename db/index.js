...
async function migrateExisting(){
  ...
  if(await tableExists('companies')) for(const [c,d] of Object.entries({rut_empresa:'TEXT',razon_social:'TEXT',region:'TEXT',comuna:'TEXT',rating:'REAL DEFAULT NULL',tamano_empresa:'TEXT',sitio_web:'TEXT',descripcion:'TEXT',documento_empresa:'TEXT',subscription_started_at:'TEXT',subscription_ends_at:'TEXT',cancel_at_period_end:'INTEGER DEFAULT 0',email_verified:(client==='postgres'?'BOOLEAN DEFAULT FALSE':'INTEGER DEFAULT 0'),email_verification_token_hash:'TEXT',email_verification_expires_at:(client==='postgres'?'TIMESTAMPTZ':'TEXT'),password_reset_token_hash:'TEXT',password_reset_expires_at:(client==='postgres'?'TIMESTAMPTZ':'TEXT'),verification_reviewed_at:(client==='postgres'?'TIMESTAMPTZ':'TEXT'),verification_review_notes:'TEXT'})) await ensureColumn('companies',c,d);
...
function companyPublic(c){
  ...
  const base={...c,plan:active?'paid':'free',plan_label:active?'Pagado':'Free',plan_price:'0,5 UF/mes',billing_period_days:30,subscription_active:active,cancel_at_period_end:Boolean(c.cancel_at_period_end),verificada:Boolean(c.verificada),email_verified:Boolean(c.email_verified),reviews_count:Number(c.reviews_count||0),rating:Number(c.reviews_count||0)>0?Number(c.rating||0):null};
...
async function getCompanyPublicById(id){ return companyPublic((await query(`SELECT c.id,c.nombre,c.razon_social,c.rut_empresa,c.region,c.comuna,c.ubicacion,c.tipo_empresa,c.necesidad,c.email,c.whatsapp,c.plan,c.subscription_started_at,c.subscription_ends_at,c.cancel_at_period_end,c.verificada,AVG(r.rating) rating,COUNT(r.id) reviews_count,c.tamano_empresa,c.sitio_web,c.descripcion,c.email_verified FROM companies c LEFT JOIN reviews r ON r.target_type='company' AND r.target_id=c.id WHERE c.id=$1 GROUP BY c.id`,[id]))[0]); }
...
async function adminListCompanies(f={}){
  ...
  const rows=await query('SELECT c.id,c.nombre,c.razon_social,c.rut_empresa,c.region,c.comuna,c.tipo_empresa,c.necesidad,c.email,c.whatsapp,c.plan,c.verificada,c.rating,c.tamano_empresa,c.sitio_web,c.descripcion,c.documento_empresa,c.verification_reviewed_at,c.verification_review_notes,c.created_at,COUNT(DISTINCT j.id) jobs_count,COUNT(DISTINCT a.id) applications_count FROM companies c LEFT JOIN jobs j ON j.company_id=c.id LEFT JOIN applications a ON a.job_id=j.id '+(where.length?'WHERE '+where.join(' AND '):'')+' GROUP BY c.id ORDER BY c.id DESC LIMIT 300',params);
...
async function adminListProfiles(f={}){
  ...
  const rows=await query('SELECT p.id,p.tipo,p.nombre,p.rut,p.region,p.comuna,p.licencia,p.experiencia,p.especialidad,p.disponibilidad,p.verificado,p.email,p.whatsapp,p.documento_licencia,p.hoja_vida_conductor,p.licencia_vencimiento,p.documento_estado,p.document_reviewed_at,p.document_review_notes,p.created_at,COUNT(t.id) trucks_count FROM profiles p LEFT JOIN trucks t ON t.profile_id=p.id '+(where.length?'WHERE '+where.join(' AND '):'')+' GROUP BY p.id ORDER BY p.id DESC LIMIT 300',params);
...
async function adminUpdateProfileVerification(id,data={}){
  const reviewedAt=iso(new Date());
  if(data.documento_estado!==undefined) await query('UPDATE profiles SET documento_estado=$1,document_reviewed_at=$2,document_review_notes=COALESCE($3,document_review_notes) WHERE id=$4',[data.documento_estado,reviewedAt,data.review_notes||null,id]);
  if(data.verificado!==undefined) await query('UPDATE profiles SET verificado=$1,document_reviewed_at=COALESCE(document_reviewed_at,$2) WHERE id=$3',[client==='postgres'?Boolean(data.verificado):(data.verificado?1:0),reviewedAt,id]);
  return (await adminListProfiles({q:''})).find(x=>String(x.id)===String(id));
}
async function adminUpdateCompanyVerification(id,data={}){
  const reviewedAt=iso(new Date());
  if(data.verificada!==undefined) await query('UPDATE companies SET verificada=$1,verification_reviewed_at=$2,verification_review_notes=COALESCE($3,verification_review_notes) WHERE id=$4',[client==='postgres'?Boolean(data.verificada):(data.verificada?1:0),reviewedAt,data.review_notes||null,id]);
