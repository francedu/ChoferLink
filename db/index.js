...
async function migrate(){
  if(client==='postgres'){
    await query(`CREATE TABLE IF NOT EXISTS profiles(id SERIAL PRIMARY KEY,tipo TEXT NOT NULL,nombre TEXT NOT NULL,rut TEXT,region TEXT,comuna TEXT,ubicacion TEXT NOT NULL,licencia TEXT,experiencia INTEGER DEFAULT 0,especialidad TEXT NOT NULL,disponibilidad TEXT NOT NULL,verificado BOOLEAN DEFAULT FALSE,email TEXT,whatsapp TEXT NOT NULL,rutas TEXT,descripcion TEXT,documento_licencia TEXT,hoja_vida_conductor TEXT,licencia_vencimiento TEXT,documento_estado TEXT DEFAULT 'pendiente',password_hash TEXT,created_at TIMESTAMPTZ DEFAULT NOW())`);
...
async function migrateExisting(){
  if(await tableExists('profiles')) for(const [c,d] of Object.entries({rut:'TEXT',region:'TEXT',comuna:'TEXT',rutas:'TEXT',descripcion:'TEXT',documento_estado:`TEXT DEFAULT 'pendiente'`,especialidad:`TEXT DEFAULT 'General'`,hoja_vida_conductor:'TEXT',licencia_vencimiento:'TEXT',password_hash:'TEXT'})) await ensureColumn('profiles',c,d);
...
function companyPublic(c){
  ...
  const base={...c,plan:active?'paid':'free',plan_label:active?'Pagado':'Free',plan_price:'0,5 UF/mes',billing_period_days:30,subscription_active:active,cancel_at_period_end:Boolean(c.cancel_at_period_end),verificada:Boolean(c.verificada),rating:Number(c.rating||0),reviews_count:Number(c.reviews_count||0)};
...
async function loginProfile(email,password){ const p=(await query('SELECT * FROM profiles WHERE LOWER(email)=LOWER($1)',[email]))[0]; if(!p||!vpw(password,p.password_hash)) return null; const tk=makeToken(); await query('INSERT INTO profile_sessions(token,profile_id) VALUES($1,$2)',[tk,p.id]); const {password_hash,...clean}=p; return {token:tk,profile:{...clean,verificado:Boolean(clean.verificado)}}; }
...
async function adminListJobs(){ return query('SELECT j.*,c.nombre empresa,c.email empresa_email,COUNT(a.id) applications_count FROM jobs j JOIN companies c ON c.id=j.company_id LEFT JOIN applications a ON a.job_id=j.id GROUP BY j.id ORDER BY j.id DESC LIMIT 300'); }
...
async function adminUpdateProfileVerification(id,data={}){
  ...
  if(data.verificado!==undefined) await query('UPDATE profiles SET verificado=$1 WHERE id=$2',[data.verificado?1:0,id]);
  return (await adminListProfiles({q:''})).find(x=>String(x.id)===String(id));
}
async function adminUpdateCompanyVerification(id,data={}){
  if(data.verificada!==undefined) await query('UPDATE companies SET verificada=$1 WHERE id=$2',[data.verificada?1:0,id]);
